...
# First SI feed lands in position 0; each subsequent shift propagates toward SO.
#
...
LOAD_BIT_ORDER = "ascending_position"
...
def load_sequence(target_by_position: dict[int, bool], chain_length: int) -> list[bool]:
    ...
    return [target_by_position.get(position, False) for position in range(chain_length)]
