...
py::list simulate_transition_incremental_py(
    ...
    const std::vector<int64_t>& fault_ids, int64_t vector_start_index,
    const std::string& unsupported_policy,
    const std::vector<std::string>& blackbox_instances, int sim_threads = 1) {
  // Release the GIL only around the pure-C++ grading; reacquire before building
  // the Python list (see simulate_incremental_py for the rationale).
  std::vector<atpg::ProgressiveDetection> detections;
  {
    py::gil_scoped_release release;
    detections = atpg::simulate_transition_incremental(
        json_path, cell_map_path, db_path, campaign_id, run_id, new_pairs,
        input_order, fault_ids, vector_start_index, unsupported_policy,
        blackbox_instances, sim_threads);
  }
  py::list out;
  for (const auto& det : detections) {
    py::dict row;
...
py::list simulate_tentative_preloaded_py(
  ...
  {
    py::gil_scoped_release release;
    detected = atpg::simulate_tentative_from_preloaded(
        json_path, cell_map_path, records, vector, input_order,
        unsupported_policy, blackbox_instances, test_mode, sim_threads);
  }
  py::list out;
...
py::list simulate_transition_tentative_preloaded_py(
  ...
  {
    py::gil_scoped_release release;
    detected = atpg::simulate_transition_tentative_from_preloaded(
        json_path, cell_map_path, records, launch, capture, input_order,
        unsupported_policy, blackbox_instances, sim_threads);
  }
  py::list out;
  for (int64_t id : detected) out.append(id);
  return out;
