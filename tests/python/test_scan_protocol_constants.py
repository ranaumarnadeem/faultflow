...
THREE_FF_TARGET = {0: True, 1: False, 2: True}
...
def test_load_order_matches_three_ff_chain_fixture() -> None:
    assert load_sequence(THREE_FF_TARGET, 3) == [True, False, True]


def test_unload_order_matches_three_ff_chain_fixture() -> None:
    assert unload_sequence(THREE_FF_TARGET, 3) == [True, False, True]


def test_unload_is_reverse_of_load_for_same_target() -> None:
    targets = [
        THREE_FF_TARGET,
...
def test_multi_chain_load_sequences_match_multichain_fixture() -> None:
    assert load_sequence(CHAIN0_TWO_FF_TARGET, 2) == [True, False]
    assert load_sequence(CHAIN1_TWO_FF_TARGET, 2) == [False, True]
...
def test_asymmetric_chain_lengths_pad_with_false() -> None:
    ...
    assert load_sequence(target, 4) == [True, False, True, False]
...
def test_ordering_constants_document_fixture_validated_convention() -> None:
    assert LOAD_BIT_ORDER == "ascending_position"
